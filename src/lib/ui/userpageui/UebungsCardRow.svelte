<script lang="ts">
    ...
    <div class="w-[100%] h-[50vh] flex justify-center items-center mt-4 ">
        ...
        {#each chunck as s}
            <div class="card flex w-[20vw] h-[40vh]">
                <ul >
                    ...
                    <li class="mt-5 text-xl w-full text-center">Level:
                            {#await getVal(s, "level") then value}
                                {value}
                            {/await}
                    </li>
                    <li class=" mt-5 text-xl w-full text-center">
                        Muscelgroup:
                        {#await getVal(s, "muscelgroup") then value}
                            {value}
                        {/await}
                    </li>
                    <li class="text-xl mt-5 w-full text-center max-w-full max-h-[10vh]">

                        {#await getVal(s, "way") then value}
                            <details class="dropdown mb-32">
                                <summary class="m-1 btn">Description:</summary>
                ...
                <div class="flex items-end justify-end  h-full w-full ">
                    <button  on:click={destroy(s)} class="tooltip">
